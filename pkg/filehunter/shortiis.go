...
import (
	...
	"github.com/ibrahmsql/iismap/pkg/levenshtein"
	"github.com/ibrahmsql/iismap/pkg/maths"
	"github.com/ibrahmsql/iismap/pkg/shortutil"
...
//
//go:embed resources/bg_comprehensive_wordlist.txt
...
func getBanner() string {
	return color.New(color.FgBlue, color.Bold).Sprint("🔍 FileHunter v"+version) + " · " + color.New(color.FgWhite, color.Bold).Sprint("advanced IIS file discovery tool")
}
...
func Run() {
	...
	if args.Wordlist != "" {
		...
		log.Info("Using built-in comprehensive wordlist")
		fh, _ := defaultWordlist.Open("resources/bg_comprehensive_wordlist.txt")
		s = bufio.NewScanner(fh)
	}
